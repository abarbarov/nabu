...
import { Branch, Commit, Project } from '../../../../proto/v1/nabu_pb';
...
type ProjectViewProps = {
  ...
  onBuild: (token: string, projectid: number, branch: string, sha: string) => void
      ...
      {props.commits.sort((a, b) => {
        ...
        >
          ...
          <div className="commits-col commits-col_actions">
            ...
            <button type="button" className="btn btn_color_orange btn_square" onClick={() => props.onCopy(props.token, props.selectedProject.id, commit.sha)} title="copy">
              <svg className="btn_svg" viewBox="0 0 24 24">
            ...
            <button type="button" className="btn btn_color_blue btn_square" onClick={() => props.onInstall(props.token, props.selectedProject.id, commit.sha, 'blue')} title="install blue">
              <svg className="btn_svg" viewBox="0 0 24 24">
                <path fill="#000000" d="M15,14V8H17.17L12,2.83L6.83,8H9V14H15M12,0L22,10H17V16H7V10H2L12,0M7,18H17V24H7V18M15,20H9V22H15V20Z" />
              </svg>
            </button>
            <button type="button" className="btn btn_color_green btn_square" onClick={() => props.onInstall(props.token, props.selectedProject.id, commit.sha, 'green')} title="install green">
              <svg className="btn_svg" viewBox="0 0 24 24">
                <path fill="#000000" d="M15,14V8H17.17L12,2.83L6.83,8H9V14H15M12,0L22,10H17V16H7V10H2L12,0M7,18H17V24H7V18M15,20H9V22H15V20Z" />
              </svg>
            </button>
            <button type="button" className="btn btn_color_blue btn_square" onClick={() => props.onRestart(props.token, props.selectedProject.id, commit.sha, 'blue')} title="reload blue">
              <svg className="btn_svg" viewBox="0 0 24 24">
            ...
            <button type="button" className="btn btn_color_green btn_square" onClick={() => props.onRestart(props.token, props.selectedProject.id, commit.sha, 'green')} title="reload green">
              <svg className="btn_svg" viewBox="0 0 24 24">
                <path fill="#000000" d="M11,4C13.05,4 15.09,4.77 16.65,6.33C19.78,9.46 19.77,14.5 16.64,17.64C14.81,19.5 12.3,20.24 9.91,19.92L10.44,17.96C12.15,18.12 13.93,17.54 15.24,16.23C17.58,13.89 17.58,10.09 15.24,7.75C14.06,6.57 12.53,6 11,6V10.58L6.04,5.63L11,0.68V4M5.34,17.65C2.7,15 2.3,11 4.11,7.94L5.59,9.41C4.5,11.64 4.91,14.39 6.75,16.23C7.27,16.75 7.87,17.16 8.5,17.45L8,19.4C7,19 6.12,18.43 5.34,17.65Z"/>
              </svg>
            </button>
